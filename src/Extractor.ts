...
type IChecker = (str: string) => boolean;
 ...
 * @param checker
 */
const filterNode: (node: postcss.AtRule, checker: IChecker) => void = (node, checker) => {
  let flag = false;
  node.walkDecls(decl => {
    if (!flag) {
...
export default class Extactor {
  ...
  private colorRegs: RegExp[] = [];
  private ruleChecker: IChecker;
