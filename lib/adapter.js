...
exports.antdSelectorAdapter = selector => {
    ...
    if (selector === '.ant-calendar-today .ant-calendar-date') {
        const calendarAdapter = (node, postCss) => {
            const newNode = node.clone({
                selector: ':not(.ant-calendar-selected-date):not(.ant-calendar-selected-day).ant-calendar-today .ant-calendar-date',
            });
            const target = postCss.rule({
                selector: ':not(.ant-calendar-selected-date):not(.ant-calendar-selected-day).ant-calendar-today .ant-calendar-date:active',
            });
            target.append({
                prop: 'color',
                value: '#ffffff',
    ...
    if (selector === '.ant-menu-horizontal > .ant-menu-item-selected > a' ||
